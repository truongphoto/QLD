(() => {
  ...
  function contextTexts(element) {
    ...
    for (let depth = 0; node && depth < 3; depth++, node = node.parentElement) {
      const labels = [...node.querySelectorAll(':scope > label, :scope > .label, :scope > [class*="label"]')].slice(0,3);
      for (const label of labels) texts.push(label.textContent);
      const previous = node.previousElementSibling;
      if (previous && previous.textContent?.trim().length < 180) texts.push(previous.textContent);
    }
